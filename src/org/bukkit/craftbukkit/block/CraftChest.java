...
public class CraftChest extends CraftBlockState implements Chest {
    ...
    public Inventory getInventory() {
        ...
        if (world.getBlockTypeIdAt(x - 1, y, z) == id) {
            CraftInventory left = new CraftInventory((net.minecraft.tileentity.TileEntityChest)world.getHandle().func_72796_p(x - 1, y, z));
            inventory = new CraftInventoryDoubleChest(left, inventory);
        }
        if (world.getBlockTypeIdAt(x + 1, y, z) == id) {
            CraftInventory right = new CraftInventory((net.minecraft.tileentity.TileEntityChest) world.getHandle().func_72796_p(x + 1, y, z));
            inventory = new CraftInventoryDoubleChest(inventory, right);
        }
        if (world.getBlockTypeIdAt(x, y, z - 1) == id) {
            CraftInventory left = new CraftInventory((net.minecraft.tileentity.TileEntityChest) world.getHandle().func_72796_p(x, y, z - 1));
            inventory = new CraftInventoryDoubleChest(left, inventory);
        }
        if (world.getBlockTypeIdAt(x, y, z + 1) == id) {
            CraftInventory right = new CraftInventory((net.minecraft.tileentity.TileEntityChest) world.getHandle().func_72796_p(x, y, z + 1));
            inventory = new CraftInventoryDoubleChest(inventory, right);
        }
        return inventory;
    }
